...
def glfind(raw_signal: NDArray, peak: NDArray[np.int64], sizes: NDArray, concentrations: NDArray) -> GLFindResult:
    ...
    complete_peaks_locations, all_local_minimums = find_signal_minima(baseline_corrected)

    complete_peaks_locations = refine_minima_near_reference_peaks(baseline_corrected, reference_peaks, pre_unrecognized_peaks, complete_peaks_locations)
    ...
    sdc = np.polyfit(peak, sizes, 5)  # калибровка по стандарту
    sdc2 = np.polyfit(sizes, peak, 5)  # калибровка в обратную сторону для проверки соотвествия границ
    ...
    if len(hidden_lib_peak_locations) == 0:
        ...
        hidden_lib_peak_locations, new_hidden_lib_areas, hidden_final_lib_local_minimums = compute_hidden_library_area(
            baseline_corrected, start_index, end_index, lib_peak_locations)
        hidden_lib_areas = np.append(hidden_lib_areas, new_hidden_lib_areas)
